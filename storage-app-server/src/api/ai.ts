...
const generateLocalAnalysis = (data: DashboardData): AnalysisResult => {
  const { metrics } = data;
...
router.post('/analyze-dashboard', async (req, res) => {
  try {
    const dashboardData: DashboardData = req.body;
...
const generateLocalReportsAnalysis = (data: any) => {
...
${products.slice(0, 10).map((p, i) => 
  `${i + 1}. ${p.descrizione || 'N/A'} - ${p.fornitore || 'N/A'} - €${p.prezzo_acquisto || 'N/A'} - ${p.utile || 'N/A'}%`
).join('\n')}
  ...
  "oddValues": 0
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        console.log(`❌ DeepSeek API attempt ${attempt} failed:`, (error as any).message);
        lastError = error;
    ...
    console.error('Product analysis error:', (error as any).message);
    res.status(500).json({ 
...
const generateLocalProductAnalysis = (products: any[], totalCount: number) => {
  ...
  products.forEach(product => {
    ...
    missingFields += requiredFields.filter(field => 
      !product[field] || product[field] === '' || product[field] === null
    ).length;
